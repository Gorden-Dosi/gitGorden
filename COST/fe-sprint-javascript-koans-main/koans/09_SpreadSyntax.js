describe('Spread syntax에 대해 학습합니다.', function () {
  it('전개 문법(spread syntax)을 학습합니다.', function () {
    ...
    const arr = [0, ...spread, 4];// ...spread - 문법이라 그냥 써야함.
...
//9장 문제에서 'Rest Parameter는 함수의 전달인자를 배열로 다룰 수 있게 합니다.'
// 부분에서 expect(returnSecondArg('only give first arg')).to.equal(undefined);
// 왜 이부분에서 undefined인지 모르겠습니다
//
//
// 9_spreadsyntax 의 89번째줄이 빈배열이 반환 되어야하지 않나요?  
// 답이 length가 있는것처럼 값이 들어있는 배열을 반환합니다.
// 일반 객체는 iterable 이나 array-like object가 아니다.
// 객체 -> 속성간의 순서가 보장되지 않는다.
// map set -- 사용가능
//
// 9장 문제에서 'Rest Parameter는 함수의 전달인자를 배열로 다룰 수 있게 합니다.
//' 부분에서 expect(returnSecondArg('only give first arg')).to.equal(undefined);
