describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    ...
    expect(emptyObj.length).to.equal(undefined); // 배열에 랭스를 알수 없음, 속성에는 길이가 없음. 배열은 요소를 길이라고 보기 애매하다.

    const megalomaniac = {
      mastermind: 'Joker',
      henchwoman: 'Harley',
      getMembers: function () {
        return [this.mastermind, this.henchwoman]; //this는 자기자신이라고 생각하면 괜찮음. 예외상황은 많음.
      },
    ...
    expect(megalomaniac.relations[3]).to.equal(undefined);
  ...
  it('Object의 속성(property)를 다루는 방법을 확인합니다.', function () {
    ...
    expect(megalomaniac['mastermind']).to.equal('Neo'); //문법
  ...
  it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
    ...
    }; //자바스크립트에서 this의 문법은 논란이 많지만 JAVA등에서부터 사용하던 사람들이 많이 사용한다.
